...
* guest `/` → main Sales Analyzer workspace (optional Company / Employee, upload, transcript, short report; full report at `/analysis/{token}/full`);
* `/login` → admin login;
...
Pipeline: upload → `TranscribeCall` (ElevenLabs Scribe v2) → `AnalyzeCall`. `AnalyzeCall` uses `AnalysisContextBuilder` then **one** structured LLM call (schema v3). A second coaching pass was considered and rejected: the v3 JSON is bounded by collection limits, two calls would duplicate the transcript and company context, and failure/latency would double. `ConversationMetricsCalculator` overwrites talk-time metrics from transcript segments after speaker-role mapping (DEC-049). The Main Analyzer UI requires an explicit Generic or Company choice before upload (DEC-063). Processing progress is derived from existing Call statuses; the operator may cancel (`cancelled`) without aborting the external provider request, but cancelled calls never continue the internal pipeline (DEC-064). Analyzer uploads persist `ui_locale` from the selected UI language; the analysis worker uses that stored locale for report language (DEC-070). STT `language_code` is normalized by BCP-47 primary subtag; UI locale is not the expected audio language (DEC-072). Analyzer uploads with `company_id` null get full generic deep analysis. Analyzer or admin uploads with a Company also receive that company’s knowledge, facts, and default active scorecard (DEC-057 / DEC-060). Context packing is UTF-8 character-safe with a 50,000-character budget (DEC-058). If transcription is not ready, analyzer upload is rejected with a generic unavailable message (no provider names). If no AI key/model is configured, the Call stays `analysis_pending`. Stored v1/v2 results remain presentable. Main Analyzer completed output is the short report; `/analysis/{public_token}/full` is a read-only Inertia view of the same `sales_analyses` row (DEC-067 / DEC-068). Empty sections are omitted from the UI (DEC-069). Gemini `responseJsonSchema` models high-value nested blocks without sending the full v3 graph (DEC-071).
