...
**Phase 0–5 are complete (application code).** `/` is the public analyzer. Admins sign in at `/login`.

Public upload queues transcription, then generic AI analysis when Settings → AI is configured. Admin company pages store sales knowledge and scorecards; company calls use that context. Live ElevenLabs/LLM smoke tests are deferred. Kit CRM screens still exist as hidden legacy routes.
