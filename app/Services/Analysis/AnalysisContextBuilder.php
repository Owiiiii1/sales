...
class AnalysisContextBuilder
{
    public function build(Call $call): AnalysisContext
    {
        ...
        $company->scorecards->load(['criteria' => fn ($query) => $query->where('is_active', true)->orderBy('sequence')]);
    ...
    /**
     * @return array{snapshot: array<string, mixed>, text: string, truncated: bool}
     */
    private function pack(Company $company, ?CompanyScorecard $scorecard): array
    {
        ...
        foreach ($chunks as $chunk) {
            ...
            if (strlen($text) <= $remaining) {
                ...
                $texts[] = $text;
                $remaining -= strlen($text) + 2;

                continue;
            }

            if ($remaining > 80) {
                $texts[] = substr($text, 0, $remaining - 20).'…[truncated]';
                $included[$chunk['key']] = $chunk['data'];
                $truncated = true;
            } else {
                $truncated = true;
    ...
    {
        return max(1000, (int) config('sales-analyzer.analysis.context_budget_characters', 24000));
    }
